...
void EntityManager::ReserveComponentStorage(std::size_t capacity) {
    ReserveFlagArrays(capacity, {&hasTag,         &hasTransform,      &hasSprite,    &hasInventory,     &hasStorage,     &hasRestSpot,
                                 ...
                                 &hasHarvestable, &hasLoot,           &hasVillage,   &hasVillageMember, &hasFamily,      &hasSocial,
                                 &hasPersonality, &hasVillageRequest, &hasWorkplace, &hasHealth,        &hasNeeds,       &hasProfession,
                                 &hasBehavior,    &hasAIContext,      &hasStats,     &hasEquipment});
    ...
    ReserveComponentArrays(capacity, tags, transforms, sprites, inventories, storages, restSpots, blueprints, constructions, rooms, costs,
                           deconstructs, doors, harvestables, loots, villages, villageMembers, families, socials, personalities,
                           villageRequests, workplaces, healths, needs, professions, behaviors, aiContexts, stats, equipments);
}
...
void EntityManager::ResetComponentFlags(EntityID id) {
    ResetFlags(id,
               {&hasTag,          &hasTransform,     &hasSprite,     &hasInventory,   &hasStorage,     &hasRestSpot,       &hasBlueprint,
                ...
                &hasVillage,      &hasVillageMember, &hasFamily,     &hasSocial,      &hasPersonality, &hasVillageRequest, &hasWorkplace,
                &hasHealth,       &hasNeeds,         &hasProfession, &hasBehavior,    &hasAIContext,   &hasStats,          &hasEquipment});
}
