...
StructureKind structure_kind_from_string(const char* name)
{
    ...
    if (strcmp(buf, "HUT_CANNIBAL") == 0 || strcmp(buf, "HUT") == 0 || strcmp(buf, "CANNIBAL_DEN") == 0)
        return STRUCT_HUT_CANNIBAL;
    if (strcmp(buf, "CRYPT") == 0)
        return STRUCT_CRYPT;
    if (strcmp(buf, "RUIN") == 0)
        return STRUCT_RUIN;
    if (strcmp(buf, "VILLAGE_HOUSE") == 0 || strcmp(buf, "HOUSE") == 0 || strcmp(buf, "HOMESTEAD") == 0)
        return STRUCT_VILLAGE_HOUSE;
    if (strcmp(buf, "TEMPLE") == 0 || strcmp(buf, "SANCTUARY") == 0)
...
void load_structure_metadata(const char* path)
{
    ...
    while (fgets(line, sizeof(line), f))
    {
        ...
        *sep = '\0';
        ...
        if (strcasecmp(key, "structure_kind") == 0 || strcasecmp(key, "structure") == 0 ||
            strcasecmp(key, "structure_id") == 0)
        {
            StructureKind explicitKind = structure_kind_from_string(value);
            if (explicitKind == STRUCT_COUNT)
                printf("⚠️  Unknown structure kind token '%s' in metadata file '%s'\n", value, path);
            current = explicitKind;
            continue;
        }
